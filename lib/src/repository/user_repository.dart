...
Future<Otp> sendOtp(Otp o) async {
  final String url = '${GlobalConfiguration().getString('api_base_url')}phone-otp-send';
  final client = new http.Client();
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
    body: json.encode(o.toMap()),
  );
  if (response.statusCode == 200) {
    setCurrentUser(response.body);
    Otp otp = Otp.fromJson(json.decode(response.body)['otp']);
    return otp;
  }
}

Future<Otp> verifyOtp(Otp o) async {
  final String url = '${GlobalConfiguration().getString('api_base_url')}phone-otp-verify';
  final client = new http.Client();
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
    body: json.encode(o.verifytoMap()),
  );
  if (response.statusCode == 200) {
    setCurrentUser(response.body);
    Otp otp = Otp.fromJson(json.decode(response.body)['otp']);
